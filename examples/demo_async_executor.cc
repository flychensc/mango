...
int main(int argc, char *argv[], char *envp[])
{
    ...
    std::shared_ptr<Builder> builder = std::make_shared<AsyncExecutorServiceBuilder>("127.0.0.1", 2012);
    Director director;
    director.setBuilder(builder);

    director.construct();

    auto p_service = std::dynamic_pointer_cast<AsyncExecutorService>(builder->getResult());
